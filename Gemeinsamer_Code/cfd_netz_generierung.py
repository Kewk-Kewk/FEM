"""Gemeinsamer_Code/cfd_netz_generierung.py

Erzeugt das Gmsh-Netz und konvertiert es ins Fluent-Format.
"""
...
from Gemeinsamer_Code.fluent_netz_schreiber import write_fluent_mesh
...
def create_geometry(params, no_boundary_layer: bool) -> None:
    ...
    gmsh.option.setNumber("Mesh.MeshSizeMax", mesh.get("mesh_size_max", 0.35 * params.plate_height_m))
    ...
    gmsh.model.mesh.field.setNumber(plate_refinement, "SizeMax", mesh.get("refinement_size_max", 0.15 * params.plate_height_m))
    gmsh.model.mesh.field.setNumber(plate_refinement, "DistMin", mesh.get("refinement_dist_min", 0.02 * params.plate_height_m))
    gmsh.model.mesh.field.setNumber(plate_refinement, "DistMax", mesh.get("refinement_dist_max", 1.0 * params.plate_height_m))

    wake = gmsh.model.mesh.field.add("Box")
    gmsh.model.mesh.field.setNumber(wake, "VIn", mesh["wake_size"])
    gmsh.model.mesh.field.setNumber(wake, "VOut", mesh.get("wake_vout", 0.35 * params.plate_height_m))
    gmsh.model.mesh.field.setNumber(wake, "XMin", domain["plate_x_min"])
    gmsh.model.mesh.field.setNumber(wake, "XMax", mesh.get("wake_xmax", 14.0 * params.plate_height_m))
    gmsh.model.mesh.field.setNumber(wake, "YMin", mesh.get("wake_ymin", -1.5 * params.plate_height_m))
    gmsh.model.mesh.field.setNumber(wake, "YMax", mesh.get("wake_ymax", 1.5 * params.plate_height_m))
    ...
    if not no_boundary_layer:
        ...
        gmsh.model.mesh.field.setNumber(boundary_layer, "ratio", mesh.get("boundary_layer_ratio", 1.2))
        gmsh.model.mesh.field.setNumber(boundary_layer, "thickness", mesh.get("boundary_layer_thickness", 0.02 * params.plate_height_m))
...
def generate_cfd_mesh(params, output_path: Path, gmsh_output_path: Path, no_boundary_layer: bool = False, show_gmsh: bool = False) -> None:
    gmsh.initialize()
    try:
        gmsh.option.setNumber("General.Terminal", 1)
        create_geometry(params, no_boundary_layer)
        gmsh.model.mesh.generate(2)
        gmsh_output_path.parent.mkdir(parents=True, exist_ok=True)
        gmsh.write(str(gmsh_output_path))
        points, cells, boundary_zones_by_edge = collect_mesh_data()
        write_fluent_mesh(output_path, points, cells, boundary_zones_by_edge)
        print(f"Wrote Gmsh mesh: {gmsh_output_path}")
        print(f"Wrote Fluent mesh: {output_path}")
