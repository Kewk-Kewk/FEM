...
def set_reference_values(solver, params) -> None:
...
def plot_pressure_distribution(frame: pd.DataFrame, path: Path) -> None:
    ...
    labels = {"front_upstream": "Vorderseite", "back_downstream": "Rueckseite"}
...
def write_summary(path: Path, params, force_report: dict[str, float]) -> None:
...
def main() -> None:
    ...
    try:
        solver.settings.file.read_case_data(file_name=str(args.case))
        set_reference_values(solver, params)
        force_report = write_force_report(solver, args.out_dir / "wall_forces.txt", args.wall)
        wall_data = get_wall_pressure_data(solver, params, args.wall)
        export_contours(solver, args.out_dir)
    finally:
        solver.exit()

    wall_data.to_csv(args.out_dir / "wall_pressure.csv", index=False)
    plot_pressure_distribution(wall_data, args.out_dir / "pressure_distribution.png")
    plot_drag_convergence(
        args.out_dir / "drag_plate.out",
        force_report,
        args.out_dir / "drag_convergence.png",
    )
    plot_mesh_from_gmsh(
        args.out_dir / "plate_2d_gmsh.msh",
        args.out_dir / "mesh_overview.png",
        args.out_dir / "mesh_plate_zoom.png",
    )
    write_summary(args.out_dir / "drag_summary_corrected.txt", params, force_report)
    write_summary(args.out_dir / "drag_summary.txt", params, force_report)

    print((args.out_dir / "drag_summary_corrected.txt").read_text(encoding="utf-8"))
    print("Wrote postprocessing outputs in", args.out_dir)
